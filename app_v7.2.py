...
def symptom_free_input_page():
    ...
    if search:
        if not symptom_input.strip():
            ...
            # Drop weak anchors like "skin" so "rashes" doesn't get forced into anchor mode
            user_anchors  = [a for a in user_anchors if a not in WEAK_ANCHORS]
            ...
            # Build active generic vocabulary (tokens + stems) for THIS query
            active_generic = set(GENERIC_SYNONYMS)
            for g in user_generics:
                gl = g.lower()
                matched = False
                for base, fam in GENERIC_FAMILY.items():
                    b = base.lower()
                    # handle simple plurals like rash→rashes, cough→coughs, etc.
                    if gl == b or gl == b + "s" or gl == b + "es":
                        active_generic |= fam
                        matched = True
                        break
                if not matched:
                    active_generic.add(gl)

            active_generic_stems = {stem(t) for t in active_generic}

            st.caption(f"generic_only={generic_only} | user_generics={user_generics} | anchors={user_anchors} | generics~{sorted(list(active_generic))[:8]}")
            ...
            for idx, row in db.iterrows():
                # --- NEW: row-level generic check (helps generic-only queries like "rash", "fever", "cough")
                row_sym_tokens = re.findall(r"[A-Za-z]{3,}", str(row["Symptoms"]).lower())
                row_sym_stems  = {stem(t) for t in row_sym_tokens}
                row_has_generic = bool(row_sym_stems & active_generic_stems)
                ...
                for raw_sym in re.split(r"[,;]", str(row["Symptoms"])):
                    ...
                    has_any_generic_here = bool(phrase_stem_set & active_generic_stems)

                    # --- NEW: for generic-only queries, require a generic actually present in the phrase ---
                    if generic_only and not (has_any_generic_here or row_has_generic):
                        continue
                    ...
                    if generic_only and (has_any_generic_here or row_has_generic):
                        score += 2.0

                    # keep only meaningful matches
                    if score >= 2.0:
                        # Row-level anchor confirmation (prevents off-topic category leakage)
                        if user_anchors:
                            row_text = f"{row.get('Condition','')} {row.get('SubCategory','')} {row.get('Primary Category','')}".lower()
                            row_tokens = re.findall(r"[A-Za-z]{3,}", row_text)
                            row_stems  = {stem(t) for t in row_tokens}
                            ua_stems   = {stem(t) for t in user_anchors}
                            # require an anchor (token or stem) somewhere in the row's labels
                            if not ((set(user_anchors) & set(row_tokens)) or (ua_stems & row_stems)):
                                continue
                        matches.add(idx)
                        break
            ...
            # --- Fallback: if generic-only and no phrase-level matches, use row-level generic match ---
            if subset.empty and generic_only:
                allback_idxs = []
                for i, r in db.iterrows():
                    toks = re.findall(r"[A-Za-z]{3,}", str(r["Symptoms"]).lower())
                    stems = {stem(t) for t in toks}
                    if stems & active_generic_stems:
                        fallback_idxs.append(i)
                if fallback_idxs:
                    subset = db.loc[fallback_idxs]
...
def evaluate_rule(rule_str: str, condition: dict, user_data: dict) -> bool:
    ...
    Evaluate Labeling Rule against the **user's** inputs:
      - symptom == X         → user selected symptom X
      - risk_flag == Y       → user ticked risk flag Y
      - cq1 == yes / cq2 == yes
...
def results_page():
    ...
    if recommendation:
        ...
        if ref_text:
             html += f"<div style='margin-top:20px; font-size:1.2rem;'> 🩺 <strong>{ref_text}</strong></div>"
        ...
        if emergency:
            st.markdown(
                f"<div class='emergency-block' style='font-size: 1.2rem; margin-bottom: 20px;' >🚨 Important: {emergency}</div>",
                unsafe_allow_html=True
            )
